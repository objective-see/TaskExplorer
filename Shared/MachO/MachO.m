...
-(BOOL)parse:(NSString*)binaryPath classify:(BOOL)shouldClassify
{
    ...
    fd = openRegularFile(binaryPath, MAX_PARSE_FILE_SIZE, &size);
    if(-1 == fd)
    {
        //bail
        goto bail;
    ...
    //       ...and into mutable data, as the parser byte-swaps headers in place
    {
        //buffer
        ...
        {
            //read
            ssize_t chunk = read(fd, (unsigned char*)buffer.mutableBytes + bytesRead, (size_t)(size - bytesRead));
            if(chunk <= 0)
            {
                //bail
                break;
        ...
        //find best slice
        // ->the slice dyld would load here; classification (e.g. encrypted) only considers this one
        {
            //offset
            __block uint64_t bestOffset = 0;

            //found?
            __block BOOL found = NO;

            //query
            if(0 == macho_best_slice_in_fd(fd, ^(const struct mach_header* slice, uint64_t sliceFileOffset, size_t sliceSize) {
                bestOffset = sliceFileOffset;
                found = YES;
            }))
            {
                //save
                if(YES == found) self.bestSliceOffset = [NSNumber numberWithUnsignedLongLong:bestOffset];
            }
        }
        ...
        //short read?
        // ->truncate (parser bounds everything by 'length')
        if(bytesRead < size)
        {
            //truncate
            buffer.length = (NSUInteger)bytesRead;
        }

        //save
        self.binaryData = buffer;
    }
...
-(BOOL)isEncrypted
{
    ...
    {
        //only the best slice (if known)
        // ->other slices (e.g. a FairPlay'd x86_64 slice) never run on this machine
        if( (nil != self.bestSliceOffset) &&
            (YES != [machoHeader[KEY_HEADER_OFFSET] isEqualToNumber:self.bestSliceOffset]) )
        {
            //skip
            continue;
        }
